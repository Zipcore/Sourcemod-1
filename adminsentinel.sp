	...
	if (client != 0)
	{
		if (GetConVarInt(Cvar_WarMode) == 1)
		{
			if (GetConVarInt(Cvar_InitState))
			{
				...
				PrintToChat(client,"\x01\x04[AdminSentinel] War Mode - Listening to enemy voice comm disabled")
				g_EnemyChat[client] = 0
				PrintToChat(client,"\x01\x04[AdminSentinel] War Mode - Listening to enemy chat disabled")
			}
		}
		else
		{
...
public Action:admin_setwarmode(client, args)
{
	...
	{
		SetConVarInt(Cvar_WarMode, 1, false, false)
	}
	else
	{
		SetConVarInt(Cvar_WarMode, 0, false, false)
	}
	return Plugin_Handled
}
