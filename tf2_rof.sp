...
public OnPluginStart()
{
	...
	CreateConVar("sm_rof_version", PLUGIN_VERSION, "Version of plugin", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY);
	g_hcvarSniperScope = CreateConVar("sm_tf2_rof_scope", "0", "Set if rof should effect sniper scope", 0, true, 0.0, true, 1.0);
	g_hcvarHuntsman = CreateConVar("sm_tf2_rof_huntsman", "0", "Set if rof should effect huntsman", 0, true, 0.0, true, 1.0);
...
public Action:Command_Rof(client, args)
{
	...
	if (CheckCommandAccess(client, "sm_rof_access_target", ADMFLAG_SLAY))
	{
		if (args < 2)
		{
			ReplyToCommand(client, "[SM] Usage: sm_rof <#userid|name> <1.0 - 6.0>");
	...
	else if (CheckCommandAccess(client, "sm_rof_access", ADMFLAG_SLAY))
	{
		if (args != 1)
		{
			ReplyToCommand(client, "[SM] Usage: sm_rof <1.0 - 6.0>");
			return Plugin_Handled;
		}
