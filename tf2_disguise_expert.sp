...
// #---------------------------------------------CVARS CONFIGURATION-------------------------------------------------#
	...
	gWPAllowed = CreateConVar("sm_disguisexpert_allow_weapons", "0", "Allow players shot while disguised?", FCVAR_NONE, true, 0.0, true, 1.0);
	gWLKAllowed = CreateConVar("sm_disguisexpert_allow_walk", "1", "Allow players walk while disguised?", FCVAR_NONE, true, 0.0, true, 1.0);
	gadmflag = CreateConVar("sm_disguisexpert_flag", "f", "If admonly is enabled, put here a flag for filter users", FCVAR_NONE);
...
// #------------------------------------------------MAKE COMMANDS----------------------------------------------------#
	RegAdminCmd("sm_disguise", CallDisguise, ADMFLAG_SLAY, "Opens up the disguise menu");
	RegAdminCmd("sm_dm", CallDisguise, ADMFLAG_SLAY, "Opens up the disguise menu");
...
public Action:CallDisguise(client, args)
{
	if(IsClientInGame(client))
	{
		...
		{
			if(DMessages != 0)	
				PrintToChat(client, "\x03[\x04SM: Disguise Expert\x03] \x01Disguise is disabled for your current class.");
